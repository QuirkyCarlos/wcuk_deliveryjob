plyCompanyID 	= 0
emptyCompanies 	= {}
RegisterNetEvent('wcuk_deliveryjob:fetchCompanyID')
AddEventHandler('wcuk_deliveryjob:fetchCompanyID', function(companyID)
	plyCompanyID = companyID
	for k,v in pairs(companyBlips) do RemoveBlip(v) end
	ESX.TriggerServerCallback('wcuk_deliveryjob:getTakenCompanies', function(ownedCompanies)
	...
	function(data, menu)
		if(data.current.value == 'confirm_purchase') then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'company_choose_name_dialog', {
				...
				ESX.TriggerServerCallback('wcuk_deliveryjob:buyCompany', function(purchased)
					if purchased then
						ShowNotifyESX((Lang['company_bought']):format(math.floor(val.price)))
						TriggerServerEvent('wcuk_deliveryjob:fetchCompanySV')
						menu.close()
						deliveryMenu = nil
					else
	...
	function(data, menu)
		if(data.current.value == 'rename_company') then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'company_rename_dialog', {
				...
				menu2.close()
				local firmName = tostring(data2.value)
				ESX.TriggerServerCallback('wcuk_deliveryjob:renameCompany', function(renamed)
					if renamed then
...
function SellCompanyMenu(id,val)
	...
	function(data, menu)
		if(data.current.value == 'confirm_sale') then
			ESX.TriggerServerCallback('wcuk_deliveryjob:sellCompany', function(sold)
				if sold then
					TriggerServerEvent('wcuk_deliveryjob:fetchCompanySV')
...
function CompanyLevelMenu(id,val)
	...
	ESX.TriggerServerCallback('wcuk_deliveryjob:getCompanyData', function(level, certifcate)
		if level ~= nil then
			...
			function(data, menu)
				...
				if(data.current.value == 'buy_certifcate') then
					ESX.TriggerServerCallback('wcuk_deliveryjob:buyCertifcate', function(certificateStatus)
...
function ShopOrderMenu(id, val)
	ESX.TriggerServerCallback('wcuk_deliveryjob:fetchShopOrders', function(orders)
		local elements = {}
		if #orders > 0 then
					...
					if level ~= nil then 
						if level >= data.current.level then 
							if data.current.certificate == false then 
								...
								SelectJobVehicle(selected.jobValue, selected.label, selected.level, selected.certificate, selected.vehicles, selected.orderData, id, val)
							elseif data.current.certificate == true and certifcate ~= nil and certifcate == 1 then 
								menu.close()
								SelectJobVehicle(data.current.jobValue, data.current.label, data.current.level, data.current.certificate, data.current.vehicles, data.current.orderData, id, val)
							else
								ShowNotifyESX(Lang['job_needs_certificate'])
							end
						else
...
function SelectJobVehicle(jobValue, label, level, certificate, vehicles, orderData, id, val)
	...
	function(data, menu)
		ESX.TriggerServerCallback('wcuk_deliveryjob:vehDepositFee', function(deposited)
			if deposited then
				...
				if jobValue == 1 or jobValue == 2 then 
					TriggerEvent('wcuk_deliveryjob:parcelDelivery', jobValue, label, level, certificate, vehicleModel, nil, id, val)
				elseif jobValue == 3 then
					TriggerEvent('wcuk_deliveryjob:highValueDelivery', jobValue, label, level, certificate, vehicleModel, id, val)
				elseif jobValue == 4 then
					orderData.taken = true
					TriggerServerEvent('wcuk_deliveryjob:updateOrderState', orderData, true)
					TriggerEvent('wcuk_deliveryjob:parcelDelivery', jobValue, label, level, certificate, vehicleModel, orderData, id, val)
				end
			else
...
RegisterNetEvent('wcuk_deliveryjob:highValueDelivery')
AddEventHandler('wcuk_deliveryjob:highValueDelivery', function(jobValue, label, level, certificate, vehModel, id, val)
...
RegisterNetEvent('wcuk_deliveryjob:parcelDelivery')
AddEventHandler('wcuk_deliveryjob:parcelDelivery', function(jobValueX, labelX, levelX, certificateX, vehModelX, orderDataX, idX, valX)
	jobValue = jobValueX
	label = labelX
	level = levelX
								...
								if deliveredParcels < maxDeliveries then
									if IsEntityAttachedToAnyPed(deliveryParcel) then 
										...
										elseif deliveredParcels == maxDeliveries then 
											...
											if jobValue == 4 then
												TriggerServerEvent('wcuk_deliveryjob:orderDeliveryDone', orderData)
											end
											SetReturnBlip(val.vehSpawn[1],val.vehSpawn[2],val.vehSpawn[3])
										end
										deliveryParcel = nil 
...
function ReturnVehAndGetPaycheck()
	...
	TriggerServerEvent('wcuk_deliveryjob:retrievePaycheck', paycheck, vehDeposit, giveDeposit)
...
function SetDeliveryRoute(jobValue)
    ...
    local deliveryDist = CalculateTravelDistanceBetweenPoints(GetEntityCoords(GetPlayerPed(-1)), deliveryPos[1], deliveryPos[2], deliveryPos[3])
    parcelPrice = math.floor(deliveryDist/3/2)
end
...
function SetTruckingRoute()
    ...
    local deliveryDist = CalculateTravelDistanceBetweenPoints(GetEntityCoords(GetPlayerPed(-1)), dropOffPos[1], dropOffPos[2], dropOffPos[3])
    palletPrice = math.floor(deliveryDist/3)
end
